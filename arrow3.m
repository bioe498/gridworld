...
if lw=='/' && ~length(LineWidthOrder)              % LineWidthOrder
  LineWidthOrder=0.5; LineWidthOrderWarning=['Undefined ',...
  'LineWidthOrder, default width (0.5) will be used'];
  warning('Arrow3:LineWidthOrder',LineWidthOrderWarning)
else
  LineWidthOrder=lw;                               % 02/09/2020
end
...
%    end
  if lw==0                               % 02/09/2020
    ...
    % lsSize=size(ls), lwSize=size(lw), cSize=size(c)
    H1=Zeros;
    for i=1:n
